            ...
            @foreach($allRoles as $key => $role)
                <div class="form-check">
                    <input class="form-check-input @if($errors->has('roles')) is-invalid @endif" type="checkbox" name="roles[]" value="{{ $role->id }}" id="role_checkbox_{{$key}}"
                           @cannot(App\Rbac\Models\Permission::ASSIGN_ROLE_FLAG, App\Rbac\Classes\MemberToRole::make($user, $role)) disabled @endcannot
                ...
                @if(isset($currentRoles) && in_array($role->id, $currentRoles))
                    @cannot(App\Rbac\Models\Permission::ASSIGN_ROLE_FLAG, App\Rbac\Classes\MemberToRole::make($user, $role))
